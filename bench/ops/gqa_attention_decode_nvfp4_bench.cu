// ninfer_gqa_attention_decode_nvfp4_bench.cu
//
// Isolated perf bench for the EXACT NVFP4 DECODE attention kernel (the `--kv-dtype nvfp4`
// route without `--sage`: NVFP4 KV cache + BF16 P/PV), the reference quality bar for the
// sage decode lane:
//   decode: gqa_attention_cached -> gqa_attention_cached_small_t_launch
//                                   -> gqa_attention_decode_nvfp4_tiled_kernel + reduce
//
// The KV cache is filled by the on-device nvfp4 fill kernel (gqa_kv_append_launch)
...
//
// Usage: ninfer_gqa_attention_decode_nvfp4_bench
...
int main(int argc, char** argv) {
    ...
    for (int context : contexts) {
        ...
        // One-shot fill of the whole cache from bf16 k/v (production nvfp4 fill kernel).
