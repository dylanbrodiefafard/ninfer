...
#include "text/qwen/tool_grammar.h"
...
void validate_pixel_pipeline(const Json& config, std::string_view resource) {
...
fi::ProcessorOptions processor_options(const FrontendResources& resources, FrontendProfile profile) {
    const Json image =
    ...
    if (profile == FrontendProfile::Qwen4 &&
        (options.image_min_pixels != 65536 || options.image_max_pixels != 16777216 ||
         options.video_min_pixels != 4096 || options.video_max_pixels != 25165824)) {
        throw std::invalid_argument("Qwen4 artifact media limits differ from the exact preview profile");
    }
    return options;
}
...
public:
    Impl(const FrontendResources& resources, bool registered_checkpoint, bool vision_enabled_,
        ...
        : chat_template(compile_chat_template(resources)),
          ...
          tool_grammar(tokenizer), processor(processor_options(resources, profile)), vision_enabled(vision_enabled_) {
        if (profile == FrontendProfile::Qwen4 &&
            !chat_template.capabilities().reasoning_effort.xhigh) {
            throw std::invalid_argument("Qwen4 preview requires its source reasoning-effort template");
        }
...
Frontend make_frontend(const FrontendResources& resources, bool vision_enabled, FrontendProfile profile) {
    return Frontend(std::make_shared<const Frontend::Impl>(resources, true, vision_enabled, profile));
}
...
Frontend FrontendTestAccess::create_component(const FrontendResources& resources,
    ...
    return Frontend(std::make_shared<const Frontend::Impl>(resources, false, vision_enabled, profile));
}
