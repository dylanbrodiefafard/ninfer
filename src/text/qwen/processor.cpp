...
namespace ninfer::text::qwen::frontend_internal {
namespace {
...
Prepared prepare_qwen4(const ChatPart& part, const ProcessorOptions& options,
                       const media::decode::Policy& policy, PreprocessStats& stats) {
    auto pixels = part.kind == ChatPartKind::Image
        ? targets::qwen4::prepare_vision_image(part.media.bytes, policy)
        : targets::qwen4::prepare_vision_video(part.media.bytes, policy);
    Prepared out;
    out.item.modality = part.kind == ChatPartKind::Image ? Modality::Image : Modality::Video;
    out.item.grid = {pixels.grid.temporal, pixels.grid.height, pixels.grid.width};
    out.item.timestamps = std::move(pixels.timestamps);
    out.patches = std::move(pixels.patches);
...
Prepared prepare_image(const ChatPart& part, const ProcessorOptions& options,
    ...
    add_budget(stats, out.item);
    enforce_media_resource_limits(stats, options);
    image = media::prepare::resize_bicubic(image, size.h, size.w);
        ...
        for (int block_x = 0; block_x < gw / kMerge; ++block_x) {
            for (int merge_y = 0; merge_y < kMerge; ++merge_y) {
                for (int merge_x = 0; merge_x < kMerge; ++merge_x) {
                    media::prepare::append_temporal_patch16(frames, block_y * kMerge + merge_y, block_x * kMerge + merge_x,
                                 out.patches);
                }
            }
        }
...
Prepared prepare_video(const ChatPart& part, const ProcessorOptions& options,
    ...
    add_budget(stats, out.item);
    enforce_media_resource_limits(stats, options);
    for (media::decode::Image& frame : video.frames) { frame = media::prepare::resize_bicubic(frame, size.h, size.w); }
...
ProcessedInput Processor::process(const std::vector<ChatMessage>& messages,
    ...
    for (const ChatPart* part : parts) {
        Prepared media;
        try {
            media = options_.profile == FrontendProfile::Qwen4
                        ? prepare_qwen4(*part, options_, policy, stats)
