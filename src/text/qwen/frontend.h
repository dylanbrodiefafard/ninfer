...
// Exact, closed checkpoint preprocessing policies. Tokenization, template rendering,
// prompt ownership and output publication are shared; media arithmetic is not inferred.
enum class FrontendProfile : std::uint8_t { Qwen3_6, Qwen4 };
...
private:
    ...
    friend class FrontendTestAccess;
    friend class EncodedHistoryPrepare;
    friend Frontend make_frontend(const FrontendResources&, bool, FrontendProfile);
};
