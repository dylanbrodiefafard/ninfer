...
void enqueue_native_decoder(std::span<const NativeLayerWeights> layers,const NativePleWeights& p,
    ...
    Tensor& qsa_workspace,cudaStream_t stream,int full_prefill_slot) {
    ...
    for(int layer=0;layer<int(layers.size());++layer) {
        ...
        if(layer%4!=3) {
            ...
            if(full_prefill_slot>=0) {
                ...
                ops::gated_delta_net_layer(mixed,w.gdn,conv_in,conv_out,state_in,state_out,block,workspace,stream);
            } else {
