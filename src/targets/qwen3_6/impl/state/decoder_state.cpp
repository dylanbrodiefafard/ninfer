...
PagedKVCacheLayout plan_cache(LayoutBuilder& builder, std::uint32_t layers, std::uint32_t capacity,
    ...
    for (std::uint32_t layer = 0; layer < layers; ++layer) {
        ...
        if (int8) {
            ...
            pool_spec.planes.push_back({DType::FP8_E4M3FN, head_dim / quant_group, kv_heads, 256});
            if (sage_pv || keep_frac < 1.0f) {
                // Per-page (64-key tile) dequantized K mean for the meansim tile proxy:
                // d -> page_offset=d/4, leading=d%4 (4 floats). F32. Allocated for
                // sage fill and for Sparge-on-exact-NVFP4 (keep_frac<1); not XAttention.
    ...
    return PagedKVCacheLayout{
        ...
        .sage_pv     = sage_pv,
        .k_mean      = nvfp4 && (sage_pv || keep_frac < 1.0f),
    };
}
...
PagedKVCache::PagedKVCache(DeviceSpan backing, const PagedKVCacheLayout& layout)
    : pool_(backing, layout.pool), layers_(layout.layers), max_context_(layout.max_context),
      ...
      quant_group_(layout.quant_group), sage_pv_(layout.sage_pv), k_mean_(layout.k_mean) {}
...
PagedKVLayerView PagedKVCache::layer_view(std::uint32_t layer, Tensor block_table) const {
    ...
    const bool nvfp4_k_mean  = quantized && dtype_ == DType::U8 && k_mean_;
    const std::size_t stride = nvfp4_k_mean ? 5ULL : (quantized ? 4ULL : 2ULL);
    ...
    return PagedKVLayerView{
        ...
        .k_mean_pages  = nvfp4_k_mean ? pool_.plane(base + 4) : Tensor(),
