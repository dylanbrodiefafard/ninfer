...
template <bool K_F16, int KPanelCols, int WuPanelCols, int BlockWarps>
cudaError_t launch_fixed(const prepare_wy_wu_config& cfg, dim3 grid, head_map qk_map) {
    ...
    cudaError_t err = cudaFuncSetAttribute(
        kernel::prepare_wy_wu_kernel<K_F16, KPanelCols, WuPanelCols, BlockWarps>,
...
cudaError_t launch_prepare_wy_wu(const prepare_wy_wu_config& cfg) {
    ...
    if (cfg.H_v == 32) {
        return cfg.private_fp16 ? launch_fixed<true, 32, 16, 4>(cfg, grid, qk_map)
                                : launch_fixed<false, 32, 16, 4>(cfg, grid, qk_map);
    }
    return cfg.private_fp16 ? launch_fixed<true, 64, 32, 8>(cfg, grid, qk_map)
                            : launch_fixed<false, 64, 32, 8>(cfg, grid, qk_map);
}
