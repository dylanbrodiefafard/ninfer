...
__device__ __forceinline__ void issue_cp_16(Smem16Tile<T, COLS> dst, const T* __restrict__ src_row0,
...
mma_16_panel(float (&D)[N_TILES][4],
             Smem16Tile<std::conditional_t<F16, __half, __nv_bfloat16>, A_STRIDE> A,
             Smem16Tile<std::conditional_t<F16, __half, __nv_bfloat16>, B_STRIDE> B, int a_row_base,
...
#pragma unroll
        for (int nt = 0; nt < N_TILES; ++nt) {
            ...
            if constexpr (F16) {
                mma_f16(D[nt][0], D[nt][1], D[nt][2], D[nt][3], af[0], af[1], af[2], af[3], bf[0],
                        bf[1]);
            } else {
...
#pragma unroll
        for (int nt = 0; nt < N_TILES; ++nt) {
            ...
            const float b0 = static_cast<float>(*V.ptr(k_off + lane_t, col));
            const float b1 = static_cast<float>(*V.ptr(k_off + lane_t + 4, col));
...
#pragma unroll
    ...
    __syncthreads();
    issue_cp_16<D_PANEL, kStateDim, THREADS>(h_view, h_chunk_in + hc_base, kStateDim, tid);
    cp_commit();
    cp_wait<0>();
    __syncthreads();
...
#pragma unroll 1
    for (int panel = 0; panel < N_D_PANELS; ++panel) {
        ...
        issue_cp_16<BT, D_PANEL, THREADS>(
...
#pragma unroll
        ...
        if (panel + 1 < N_D_PANELS) {
            issue_cp_16<D_PANEL, kStateDim, THREADS>(
                h_view,
