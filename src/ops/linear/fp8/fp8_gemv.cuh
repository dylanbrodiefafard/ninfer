...
__global__ __launch_bounds__(Schedule::kThreads, Schedule::kMinBlocksPerSm) void fp8_gemv_kernel(
    ...
    Scales row_scales, Output output, RowPolicy row_policy = {},
    Epilogue epilogue = {}) {
...
#pragma unroll
            for (int local_row = 0; local_row < kStoredRowsPerWarp; ++local_row) {
                ...
                const float gate = epilogue.apply(
                    gate_row, 0, totals[local_row] * fp8_weight_scale(row_scales, gate_row));
                const float up = epilogue.apply(up_row, 0,
                                                totals[kStoredRowsPerWarp + local_row] *
                                                    fp8_weight_scale(row_scales, up_row));
...
#pragma unroll
            ...
            if (lane == 0) {
                ...
                const float value =
                    epilogue.apply(parent_row, 0, total * fp8_weight_scale(row_scales, parent_row));
