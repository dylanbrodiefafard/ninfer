...
template <int Hidden, int BlocksPerToken, int Threads>
void launch_fp8(const Tensor& ids, const Weight& table, Tensor& out, cudaStream_t stream) {
    const int grid = ids.ne[0] * BlocksPerToken;
    embed_gather_fp8_kernel<Hidden, BlocksPerToken, Threads><<<grid, Threads, 0, stream>>>(
...
void embed_gather_fp8_launch(const Tensor& ids, const Weight& table, Tensor& out,
    ...
    const std::int32_t T = ids.ne[0];
    if (table.k == 2560) {
        if (T <= 48) launch_fp8<2560, 10, 32>(ids, table, out, stream);
        else launch_fp8<2560, 1, 256>(ids, table, out, stream);
    } else if (T <= 48) {
        launch_fp8<5120, 10, 32>(ids, table, out, stream);
    } else {
        launch_fp8<5120, 1, 256>(ids, table, out, stream);
    }
    CUDA_CHECK(cudaGetLastError());
}
