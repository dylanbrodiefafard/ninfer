...
__global__ void bf16_project_kernel(const __nv_bfloat16* x, const __nv_bfloat16* weight,
                                    __nv_bfloat16* out, int rows, int columns) {
    ...
    if (threadIdx.x == 0) { out[row] = __float2bfloat16_rn(sums[0]); }
}
...
__global__ void core_query_norm_rope_kernel(const __nv_bfloat16* raw,
    ...
    if (d < 64) {
        const int pair = d & 31;
        result = rotated(normalized[pair], normalized[pair + 32], pair, d >= 32, position);
    }
    output[d + 256 * head] = __float2bfloat16_rn(result);
}
...
__global__ void output_gate_kernel(const __nv_bfloat16* attention,
    ...
    if (index >= 24 * 256) { return; }
    const int head = index / 256;
    const int d    = index % 256;
    const float gate = __bfloat162float(raw_query_gate[head * 512 + 256 + d]);
...
void qsa_bf16_project_launch(const Tensor& x, const Weight& weight, Tensor& out,
                             cudaStream_t stream) {
    bf16_project_kernel<<<weight.n, 256, 0, stream>>>(
...
void qsa_core_norm_rope_launch(const Tensor& raw_query_gate, const Tensor& raw_key,
                               ...
                               cudaStream_t stream) {
    core_query_norm_rope_kernel<<<24, 256, 0, stream>>>(
    ...
    CUDA_CHECK(cudaGetLastError());
    core_key_norm_rope_kernel<<<2, 256, 0, stream>>>(
...
void qsa_output_gate_launch(const Tensor& attention, const Tensor& raw_query_gate, Tensor& gated,
                            cudaStream_t stream) {
    output_gate_kernel<<<24, 256, 0, stream>>>(
