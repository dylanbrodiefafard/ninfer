...
template <typename Geometry, typename Code, int CodeLeading>
__device__ __forceinline__ void gqa_kv_compact_copy_codes(Code* __restrict__ plane,
    ...
    for (int head = 0; head < Geometry::KVHeads; ++head) {
        Code* src =
            plane + paged_kv_element_offset<CodeLeading, Geometry::KVHeads>(src_page, head,
                                                                            src_off, 0);
        Code* dst =
            plane + paged_kv_element_offset<CodeLeading, Geometry::KVHeads>(dst_page, head,
                                                                            dst_off, 0);
        for (int d = static_cast<int>(threadIdx.x); d < CodeLeading;
...
template <typename Geometry, typename Scale, int ScaleLeading>
__device__ __forceinline__ void gqa_kv_compact_copy_scales(Scale* __restrict__ plane,
                                                           std::int32_t src_page,
                                                           std::int32_t dst_page,
                                                           std::int32_t src_off,
                                                           std::int32_t dst_off) {
    for (int head = 0; head < Geometry::KVHeads; ++head) {
        Scale* src = plane + paged_kv_element_offset<ScaleLeading, Geometry::KVHeads>(
                                 src_page, head, src_off, 0);
        Scale* dst = plane + paged_kv_element_offset<ScaleLeading, Geometry::KVHeads>(
                                 dst_page, head, dst_off, 0);
        for (int g = static_cast<int>(threadIdx.x); g < ScaleLeading;
...
template <typename Geometry, typename Code, int CodeLeading, typename Scale, int ScaleLeading,
          bool HasScale>
__global__ void gqa_kv_compact_path_kernel(Code* __restrict__ cache_k, Code* __restrict__ cache_v,
                                           Scale* __restrict__ cache_k_scale,
                                           Scale* __restrict__ cache_v_scale,
    ...
    for (std::int32_t i = 0; i < count; ++i) {
        ...
        if (src_pos != dst_pos) {
            ...
            gqa_kv_compact_copy_codes<Geometry, Code, CodeLeading>(cache_k, src_page, dst_page,
                                                                   src_off, dst_off);
            gqa_kv_compact_copy_codes<Geometry, Code, CodeLeading>(cache_v, src_page, dst_page,
                                                                   src_off, dst_off);
            if constexpr (HasScale) {
                gqa_kv_compact_copy_scales<Geometry, Scale, ScaleLeading>(
                    cache_k_scale, src_page, dst_page, src_off, dst_off);
                gqa_kv_compact_copy_scales<Geometry, Scale, ScaleLeading>(
                    cache_v_scale, src_page, dst_page, src_off, dst_off);
            }
        }
        __syncthreads();
    }
