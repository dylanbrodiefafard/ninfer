...
__launch_bounds__(WarpsPerCta * 32, MinBlocksPerSm) __global__
    ...
    constexpr std::size_t ArenaBytes =
        2 * Bc * CodeW + Br * P4Row + Br * 4 + D * P4Row + D * 4 + Bc * Groups;
...
#pragma unroll
    ...
    auto issue_kv_tile = [&](int tile_k0, int physical_page) {
        ...
        // d-major V scales: [d 256][4 16-key blocks]. A block whose keys straddle
        // split_end holds the running-max scale of the written subset; unwritten
        // keys contribute zero (their P codes are masked to 0).
