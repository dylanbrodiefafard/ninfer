...
The seven GGML identities support the byte-preserving Qwen4 architecture-verification artifact.
Their registration does not register that model with the target registry or
...
the block-scaled `NVFP4` format, row-scaled `FP8_E4M3FN_ROW_BF16S`, and seven exact GGML block
...
The thirteen quantized names above identify schemes in this sense. Their meanings are immutable: a
...
The seven GGML block formats likewise have exact decode contracts in Section 3.5 but no
...
`blockscale-k16-m128x4-v1` for `NVFP4`, `row-scale-v1` for `FP8_E4M3FN_ROW_BF16S`, and
`ggml-block-row-v1` for the seven GGML formats. Their byte order, plane packing, padding, swizzle,
divisor placement, and alignment rules belong to the layout registry, not to these sixteen numeric
formats.
...
For a positive logical matrix [N,K], W[n,k] is the exact signed E4M3FN value
decoded using Section 3.3, multiplied by the exact BF16 row scale s[n].
All 254 finite code words are admitted, including both signs of zero. Both NaN
codes (0x7f, 0xff) are forbidden. Scales must be nonnegative finite BF16 words;
negative zero is forbidden. A zero scale requires only signed-zero codes in that row.
The scale is a multiplier, not a divisor. There is no activation scale, zero point,
extra matrix divisor or implied intermediate BF16 materialization in this format.

The row-scale-v1 codec preserves code and scale words exactly. Selected mixed
Qwen3.8 weights are copied from their source artifact without requantization.
Independent operator oracles decode the code and stored scale mathematically,
then evaluate the complete Op on represented public inputs in FP32/FP64.
Private A8 activation quantization is an implementation profile, not part of this
persistent weight identity. Layout producers check all code and scale values;
the runtime reader checks framing and geometry, as for the other trusted artifacts.

### 3.5 Exact GGML block formats
...
- for `FP8_E4M3FN_ROW_BF16S`, emit finite signed E4M3FN codes and valid nonnegative
  BF16 row multipliers, with positive-zero scales restricted to zero-code rows
  under Section 3.4;
- for an exact GGML format, copy complete source blocks unchanged and preserve the final-axis block
  boundaries defined in Section 3.5;
...
Section 3.3, legal row-scaled FP8 words under Section 3.4, and exact copied GGML blocks under
Section 3.5. Section 7 governs only a producer claiming the canonical grouped signed-integer encoder
...
`NVFP4` words and divisor according to Section 3.3, row-scaled FP8 according to Section 3.4, and
exact GGML blocks according to Section 3.5. It may choose its private fusion, reduction, staging,
and intermediate precision; the observable Op result is qualified against the independent oracle
with the Op's named criterion for that implementation profile. Kernel implementation details do
...
This registry does not say where any of the sixteen formats are used. A checkpoint numeric-format
...
- any other persistent FP8, FP4, microscaling, or shared-exponent format; `NVFP4` registers only
  the exact E2M1/E4M3FN/divisor contract in Section 3.3 and the row-scaled FP8 identity
  registers only the E4M3FN/BF16-multiplier contract in Section 3.4;
...
Section 3.5; the pinned implementation is a cross-check, not the oracle. An NVFP4 or GGML Op oracle
...
- persistent quantized weights use the four grouped signed-integer identities, exact `NVFP4`,
  `FP8_E4M3FN_ROW_BF16S`, or one of the seven exact source-block GGML identities; no spelling
  constructs another scheme;
...
- `NVFP4` uses E2M1 codes, one E4M3FN scale per K-axis group of 16, and one positive FP32 matrix
  divisor under Section 3.3;
- `FP8_E4M3FN_ROW_BF16S` uses finite E4M3FN codes and one nonnegative BF16 multiplier per row under
  Section 3.4;
- the seven GGML identities retain their exact source block bytes and decode under Section 3.5;
