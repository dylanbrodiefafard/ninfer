 ...
 *
 *   position[d,p] = CastBF16(sum_{c=0..3} table[d,indices[c,p]] * weights[c,p])
 *   ideal[d,p]    = CastBF16(x[d,p] + position[d,p]).
 *
 ...
 * result with the two explicit BF16 cast boundaries above; output storage rounding belongs to the
 * Op's numerical criterion. The interpolation reduction may use its natural accumulator and
 * association. Inputs must not overlap x. The Op updates all of x in place and uses no workspace
