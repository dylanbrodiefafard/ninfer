...
#include "text/qwen/chat_template.h"
#include "text/qwen/processor.h"
#include "text/qwen/test_access.h"
#include "text/qwen/tokenizer.h"
#include "targets/qwen4/vision_frontend.h"
#include "text/qwen/official_tokenizer_dir.h"
...
using Frontend          = ninfer::text::qwen::Frontend;
using FrontendFactory   = ninfer::text::qwen::FrontendTestAccess;
using FrontendResources = ninfer::text::qwen::FrontendResources;
using PublishedOutput   = ninfer::text::qwen::PublishedOutput;
namespace fi            = ninfer::text::qwen::frontend_internal;
    ...
    failures +=
        check(header != std::string::npos && generated.rewrite_checkpoint &&
                  generated.rewrite_checkpoint->kind ==
                      ninfer::text::qwen::RewriteCheckpointKind::TurnClosure &&
...
int test_reasoning_effort_empty_history_think() {
    ...
    failures += check(empty_replay.rewrite_checkpoint &&
                          empty_replay.rewrite_checkpoint->kind ==
                              ninfer::text::qwen::RewriteCheckpointKind::ResponseReplay &&
...
int test_video_prepare(const Frontend& frontend) {
    ...
    if (!prepared_data.vision_items.empty()) {
        const auto& item = prepared_data.vision_items.front();
        failures +=
            check(item.modality == ninfer::text::qwen::PromptModality::Video &&
...
int test_declared_tool_publication() {
    ...
    failures += check(ninfer::text::qwen::unconstrained_tool_call_names(
...
int test_qwen4_media_profile() {
    using Profile = ninfer::text::qwen::FrontendProfile;
    auto source = resources(reasoning_effort_template_source());
    auto image = nlohmann::json::parse(source.preprocessor_config_json);
    image["size"]["shortest_edge"] = 65536;
    source.preprocessor_config_json = image.dump();
    const auto frontend = FrontendFactory::create_component(source, true, Profile::Qwen4);
    auto input = image_input();
    input.options.enable_thinking = false;
    const auto prompt = frontend.prepare(input);
    const auto& data = FrontendFactory::inspect(prompt);
    const auto reference = ninfer::targets::qwen4::prepare_vision_image(
        gradient_ppm(), ninfer::media::decode::Policy{});
    int failures = check(data.vision_items.size() == 1 && data.patches == reference.patches,
        "Qwen4 shared prompt lost exact source uint8 pixel preparation");
    failures += check(data.prepare.raw_patches == 256 && data.prepare.vision_tokens == 64 &&
        data.vision_items.front().grid.height == 16 && data.vision_items.front().grid.width == 16 &&
        data.vision_items.front().token_spans.front().count == 64 &&
        frontend.count_tokens(input) == data.token_ids.size(),
        "Qwen4 prompt image expansion/count/grid differs from source");
    const std::array grids{reference.grid};
    const auto positions = ninfer::targets::qwen4::prepare_multimodal_positions(data.token_types, grids, {});
    failures += check(data.positions == std::vector<std::int32_t>(
        positions.values.begin() + data.token_ids.size(), positions.values.end()) &&
        data.rope_delta == positions.rope_delta,
        "Qwen4 shared prompt RoPE positions differ from target source policy");
    auto video_input = image_input();
    video_input.options.enable_thinking = false;
    video_input.messages.front().parts.front().media.kind = ninfer::MediaKind::Video;
    // A still-image container has only one decoded frame. The exact Qwen4 video
    // frontend requires at least one temporal pair, unlike the dense-family policy.
    try { (void)frontend.prepare(std::move(video_input)); ++failures; }
    catch (const std::invalid_argument&) {}
    const auto raw = frontend.prepare_tokens({0,248046});
    try { (void)frontend.prepare_tokens({248077}); ++failures; }
    catch (const std::out_of_range&) {}
    auto invalid = source;
    invalid.preprocessor_config_json = resources().preprocessor_config_json;
    try { (void)FrontendFactory::create_component(invalid, true, Profile::Qwen4); ++failures; }
    catch (const std::invalid_argument&) {}
    return failures;
}
...
int main() {
    ...
    failures += test_qwen4_media_profile();
