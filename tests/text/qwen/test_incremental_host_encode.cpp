...
using EncodedHistoryPrepare = ninfer::text::qwen::EncodedHistoryPrepare;
...
int test_loop_pos_and_splice() {
    ...
    failures += check(replay && replay->input_ids == tokenizer.encode(committed.text) &&
        ...
        .kind = ninfer::text::qwen::RewriteCheckpointKind::TurnClosure,
...
struct CachedCall {
    ninfer::text::qwen::PreparedPrompt prompt;
