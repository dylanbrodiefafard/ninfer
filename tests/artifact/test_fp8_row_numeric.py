...
def test_all_finite_signed_codes_against_independent_scalar_oracle():
    words = [word for word in range(256) if (word & 127) != 127]
    codes = torch.tensor([words, words], dtype=torch.uint8)
    scales = _bf16_words(0x3F80, 0x3F00)
    payload = encode_fp8_row_scaled(codes, scales, codes.shape)
    values = dequantize_fp8_row_scaled(payload, codes.shape)
    expected = []
